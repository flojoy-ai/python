...
from .utils import send_to_socket, flojoy_print
    ...
    try:
        for prev_job in previous_jobs:
            ...
            if result is not None:
                flojoy_print(f"got job result from {prev_job_id}", flush=True)
                if multiple:
        ...
        flojoy_print(traceback.format_exc(), flush=True)
...
def flojoy(
    ...
    *,
    ...
    def SINE(dc_inputs:list[DataContainer], params:dict[str, Any]):
        ...
        flojoy_print('params passed to SINE', params)
    ...
    ```
    pj_ids = [123, 456]
    flojoy_print(SINE(previous_job_ids = pj_ids, mock = True))
    ```
    """
    ...
    def decorator(func: Callable[..., DataContainer | dict[str, Any]]):
        ...
        def wrapper(
            ...
            try:
                ...
                flojoy_print("previous jobs:", previous_jobs)
                ...
                flojoy_print(f"constructing inputs for {func.__name__}", flush=True)
                args: dict[str, Any] = {}
                sig = signature(func)
                ...
                flojoy_print(node_id, " params: ", args.keys(), flush=True)
                ...
                flojoy_print("error occured while running the node", flush=True)
                flojoy_print(traceback.format_exc(), flush=True)
                raise e
